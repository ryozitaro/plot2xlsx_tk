...
class XlsxOut:
    ...
    def _write_file_data(self, cell: str|tuple[int, int], p_or_s: str, df: pd.DataFrame) -> None:
        """
    ...
    def _create_chart(self, cell: str|tuple[int, int], data_length: int):
        ...
        chart.add_series({
            ...
            "values": [self.ws.get_name(), row, col+2, row+data_length, col+2],
        })
        chart.add_series({
            "name": "in",
            "categories": [self.ws.get_name(), row, col, row+data_length, col],
            "values": [self.ws.get_name(), row, col+1, row+data_length, col+1],
            "y2_axis": True
        })
        chart.set_x_axis({
            "crossing": "max",
    ...
    def _write_sel_df(self, cell: str|tuple[int, int]) -> None:
        """
        ...
        # 書き込みデータの用意
        data = [((None,)*2, self.sel_df.index)] + list(self.sel_df.items())

        # 書き込まれるセルの特定
        cell_idx = self.sel_df.copy()
        ...
        # セルのボーダー
        border_btm_empty = self.wb.add_format({"border": 1, "bottom": 0, "font_size": 9})
    ...
    def main_write(
        self, sel_df: pd.DataFrame, p_folder: FolderData, s_folder: FolderData, img: io.BytesIO
    ) -> None:
        self.sel_df = sel_df
        ...
        ws.write("A2", "P波")
        ws.insert_image("A3", "", {"image_data": self.p_folder.bmp, "x_scale": 0.8, "y_scale": 0.8})
        ws.write("E2", "P波")
        ws.insert_image("E3", "", {"image_data": self.s_folder.bmp, "x_scale": 0.8, "y_scale": 0.8})

        self._write_file_data("J1", "P", self.p_folder.df)
        self._write_file_data("N1", "S", self.s_folder.df)

        ws.write("A16", "P波")
        chart = self._create_chart("K2", len(self.p_folder.df))
        ws.insert_chart("A17", chart)

        ws.write("A35", "S波")
        chart = self._create_chart("O2", len(self.s_folder.df))
        ws.insert_chart("A36", chart)

        self._write_sel_df("B58")
